...
watch(() => props.selectedLocation, loadInmuebles)
watch(() => props.filters, loadInmuebles, { deep: true })
  ...
  inmueblesToShow.forEach(inmueble => {
    const marker = L.marker([inmueble.lat, inmueble.lng], {
      icon: L.divIcon({
        ...
        html: `<div class="w-4 h-4 rounded-full border-2 ${getStatusColor(inmueble.estado)} shadow-lg"></div>`,
        iconSize: [16, 16],
        iconAnchor: [8, 8]
      })
    })
    
    marker.on('click', () => emit('property-selected', inmueble))
